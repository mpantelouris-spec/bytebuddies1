...
export class BLEFlasher {
  ...
  _report(percent, message, stage = 'flashing', metadata = null) {
    const progress = { percent: Math.min(100, Math.max(0, percent)), message, stage };
    if (metadata) Object.assign(progress, metadata);
  ...
  async flash(hexString, device) {
    ...
    try {
      ...
      for (let offset = 0; offset < total; offset += 16) {
        ...
        const pktNum = Math.floor(offset / 16) + 1;
        const pktTotal = Math.ceil(total / 16);
        const pct = 50 + Math.floor(((offset + 16) / total) * 45);
        this._report(
          Math.min(95, pct),
          `Packet ${pktNum} of ${pktTotal} — waiting for OK…`,
          'flashing',
          { packetIndex: pktNum, packetTotal: pktTotal },
        );
      }
      ...
      this._report(98, 'Restarting micro:bit…', 'finalizing');
      try {
        await char.writeValueWithoutResponse(new Uint8Array([CMD_RESET, 0x00]));
      } catch { /* device may already be resetting */ }
      ...
      this._report(100, 'BLE flash complete — micro:bit restarting.', 'completed');
