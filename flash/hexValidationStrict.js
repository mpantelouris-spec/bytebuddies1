...
/**
 * Validate data-record order for DAPLink.
 * Official micro:bit V2 hex legitimately revisits low flash after an upper segment
 * (same ELA) and uses a separate 0x1000 ELA bank for NVM — not a single monotonic range.
 */
function validateAddressOrder(records) {
  ...
  for (const record of records) {
    if (record.type === 0x04) {
      currentEla = (record.data[0] << 8) | record.data[1];
      lastAddress = -1;
      continue;
    }
    if (record.type !== 0x00) continue;

    const currentAddress = record.absoluteAddress;

    if (lastAddress >= 0 && currentAddress < lastAddress) {
      const inLowFlash = currentAddress < 0x00100000 && lastAddress < 0x00100000;
      const v2NvmBank = currentAddress >= 0x10000000 || lastAddress >= 0x10000000;
      // Allow micro:bit layout: main flash ↔ NVM bank, or file-order revisit of low flash.
      if (!inLowFlash && !v2NvmBank) {
        return {
          valid: false,
          error: `Address jump backwards at line ${record.line}: 0x${currentAddress.toString(16)} < 0x${lastAddress.toString(16)}.`,
