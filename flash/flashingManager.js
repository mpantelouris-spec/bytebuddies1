...
import { canPartialFlashOverBle } from './hexUtils.js';
...
export class FlashingManager {
  ...
  async _flashAuto(hexString, deviceId, board, onProgress) {
    ...
    if (typeof navigator !== 'undefined' && (navigator.usb || navigator.bluetooth)) {
      try {
        const dual = getDualModeFlashManager();
        return await dual.flash(hexString, {
          method: 'dual-auto',
          board,
          onProgress,
        });
        ...
        if (canPartialFlashOverBle(hexString) && navigator.bluetooth) {
          report(48, `USB failed — ${msg.slice(0, 60)}`, 'preparing');
        }
        report(50, `Flash failed (${msg.slice(0, 80)}), downloading .hex…`, 'preparing');
        return this._flashDownload(hexString, onProgress);
      }
    }
