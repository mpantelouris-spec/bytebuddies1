...
/* ─── Category to Color Mapping (Bright Neon Colors) ─── */
...
export function getCategoryColor(category) {
  return CATEGORY_COLOR_MAP[category] || '#f59e0b';
}

/* ─── Block type definitions ─── */
export const BLOCK_DEFS = {
  'event-start':      { label: 'When ▶ clicked', icon: '🚩', color: '#FFD700', category: 'event', params: {} },
  'event-keypress':   { label: 'On key press', icon: '🎯', color: '#FFD700', category: 'event', params: { key: 'space' } },
  'event-click':      { label: 'On click', icon: '🖱️', color: '#FFD700', category: 'event', params: {} },
  'event-collision':  { label: 'On collision', icon: '💥', color: '#FFD700', category: 'event', params: { with: 'any' } },
  ...
  'loop-forever':     { label: 'Forever', icon: '🔁', color: '#FF6B35', category: 'loop', params: {} },
  ...
  'action-print':     { label: 'Print', icon: '💬', color: '#FF3366', category: 'action', params: { message: '"Hello!"' } },
  'action-ask':       { label: 'Ask and wait', icon: '💬', color: '#FF3366', category: 'action', params: { prompt: '"What is your name?"' } },
  'action-alert':     { label: 'Alert', icon: '💬', color: '#FF3366', category: 'action', params: { message: '"Notice"' } },
  'math-add':         { label: 'Add / Subtract', icon: '🔢', color: '#00B4FF', category: 'math', params: { a: '1', op: '+', b: '1' } },
  'math-mult':        { label: 'Multiply / Divide', icon: '🔢', color: '#00B4FF', category: 'math', params: { a: '2', op: '×', b: '3' } },
  'math-random':      { label: 'Random number', icon: '🔢', color: '#00B4FF', category: 'math', params: { min: '1', max: '100' } },
  'math-round':       { label: 'Round / Abs', icon: '🔢', color: '#00B4FF', category: 'math', params: { op: 'round', value: '3.7' } },
  ...
  'sprite-changex':   { label: 'Change X by', icon: '🎭', color: '#00D9FF', category: 'sprite', params: { amount: '10' } },
  'sprite-changey':   { label: 'Change Y by', icon: '🎭', color: '#00D9FF', category: 'sprite', params: { amount: '10' } },
  'sprite-setsize':   { label: 'Set size to', icon: '🎭', color: '#00D9FF', category: 'sprite', params: { size: '100' } },
  'sprite-show':      { label: 'Show', icon: '🎭', color: '#00D9FF', category: 'sprite', params: {} },
  'sprite-hide':      { label: 'Hide', icon: '🎭', color: '#00D9FF', category: 'sprite', params: {} },
  'sprite-say':       { label: 'Say', icon: '🎭', color: '#00D9FF', category: 'sprite', params: { text: '"Hi!"', secs: '2' } },
  'control-wait':     { label: 'Wait', icon: '⏱', color: '#FF6B35', category: 'control', params: { secs: '1' } },
  'control-stop':     { label: 'Stop all', icon: '⏹', color: '#FF6B35', category: 'control', params: {} },
  ...
  // Sensing
  'sense-touching':     { label: 'Touching edge?', icon: '🔍', color: '#00FFAA', category: 'sensing', params: {} },
  'sense-touching-sprite': { label: 'Touching sprite?', icon: '🔍', color: '#00FFAA', category: 'sensing', params: { sprite: 'any' } },
  'sense-key':          { label: 'Key pressed?', icon: '⌨️', color: '#00FFAA', category: 'sensing', params: { key: 'space' } },
  'sense-mouse-x':      { label: 'Mouse X', icon: '🖱️', color: '#00FFAA', category: 'sensing', params: {} },
  'sense-mouse-y':      { label: 'Mouse Y', icon: '🖱️', color: '#00FFAA', category: 'sensing', params: {} },
  'sense-distance':     { label: 'Distance to mouse', icon: '📏', color: '#00FFAA', category: 'sensing', params: {} },
  'sense-timer':        { label: 'Timer', icon: '⏱️', color: '#00FFAA', category: 'sensing', params: {} },
  'sense-reset-timer':  { label: 'Reset timer', icon: '⏱️', color: '#00FFAA', category: 'sensing', params: {} },
  // Looks
  'looks-say':          { label: 'Say', icon: '💬', color: '#FF00FF', category: 'looks', params: { text: 'Hello!', secs: '2' } },
  'looks-think':        { label: 'Think', icon: '💭', color: '#FF00FF', category: 'looks', params: { text: 'Hmm...', secs: '2' } },
  'looks-costume':      { label: 'Switch costume to', icon: '👗', color: '#FF00FF', category: 'looks', params: { costume: '1' } },
  'looks-next-costume': { label: 'Next costume', icon: '👗', color: '#FF00FF', category: 'looks', params: {} },
  'looks-color-effect': { label: 'Set color effect', icon: '🎨', color: '#FF00FF', category: 'looks', params: { value: '0' } },
  'looks-ghost-effect': { label: 'Set ghost effect', icon: '👻', color: '#FF00FF', category: 'looks', params: { value: '0' } },
  'looks-clear-effects':{ label: 'Clear all effects', icon: '✨', color: '#FF00FF', category: 'looks', params: {} },
  'looks-grow':         { label: 'Grow by', icon: '🔼', color: '#FF00FF', category: 'looks', params: { amount: '10' } },
  'looks-shrink':       { label: 'Shrink by', icon: '🔽', color: '#FF00FF', category: 'looks', params: { amount: '10' } },
  'looks-front':        { label: 'Go to front layer', icon: '🎨', color: '#FF00FF', category: 'looks', params: {} },
  'looks-back':         { label: 'Go to back layer', icon: '🎨', color: '#FF00FF', category: 'looks', params: {} },
  // Physics
  'physics-velocity':   { label: 'Set velocity', icon: '💨', color: '#00FFDD', category: 'physics', params: { vx: '5', vy: '0' } },
  'physics-gravity':    { label: 'Set gravity', icon: '⬇️', color: '#00FFDD', category: 'physics', params: { amount: '0.5' } },
  'physics-bounce':     { label: 'Bounce off edges', icon: '↩️', color: '#00FFDD', category: 'physics', params: {} },
  'physics-friction':   { label: 'Set friction', icon: '🔄', color: '#00FFDD', category: 'physics', params: { amount: '0.9' } },
  'physics-jump':       { label: 'Jump with power', icon: '⬆️', color: '#00FFDD', category: 'physics', params: { power: '10' } },
  'physics-push':       { label: 'Push in direction', icon: '➡️', color: '#00FFDD', category: 'physics', params: { direction: '0', force: '5' } },
  // Game
  'game-score-add':     { label: 'Add to score', icon: '🏆', color: '#00FF41', category: 'game', params: { amount: '10' } },
  'game-score-set':     { label: 'Set score to', icon: '🏆', color: '#00FF41', category: 'game', params: { value: '0' } },
  'game-lose-life':     { label: 'Lose a life', icon: '❤️', color: '#00FF41', category: 'game', params: {} },
  'game-set-lives':     { label: 'Set lives to', icon: '❤️', color: '#00FF41', category: 'game', params: { value: '3' } },
  'game-over':          { label: 'Game Over', icon: '💀', color: '#00FF41', category: 'game', params: {} },
  'game-win':           { label: 'You Win!', icon: '🎉', color: '#00FF41', category: 'game', params: {} },
  'game-next-level':    { label: 'Next Level', icon: '⬆️', color: '#00FF41', category: 'game', params: {} },
  'game-spawn':         { label: 'Spawn clone of', icon: '✨', color: '#00FF41', category: 'game', params: { sprite: 'this' } },
  'game-destroy':       { label: 'Destroy this', icon: '💥', color: '#00FF41', category: 'game', params: {} },
  'game-pause':         { label: 'Pause game', icon: '⏸️', color: '#00FF41', category: 'game', params: {} },
  // Motion extras
  'motion-glide':       { label: 'Glide to', icon: '🕊️', color: '#00D9FF', category: 'motion', params: { x: '0', y: '0', secs: '1' } },
  'motion-point':       { label: 'Point toward mouse', icon: '🎯', color: '#00D9FF', category: 'motion', params: {} },
  'motion-point-dir':   { label: 'Point in direction', icon: '🎯', color: '#00D9FF', category: 'motion', params: { direction: '90' } },
  'motion-setx':        { label: 'Set X to', icon: '🎭', color: '#00D9FF', category: 'motion', params: { x: '0' } },
  'motion-sety':        { label: 'Set Y to', icon: '🎭', color: '#00D9FF', category: 'motion', params: { y: '0' } },
  'motion-speed':       { label: 'Set speed to', icon: '⚡', color: '#00D9FF', category: 'motion', params: { speed: '5' } },
  // My Blocks
  'myblock-define':     { label: 'Define my block', icon: '🧩', color: '#00D9FF', category: 'myblocks', params: { name: 'my block' } },
  'myblock-run':        { label: 'Run my block', icon: '🧩', color: '#00D9FF', category: 'myblocks', params: { name: 'my block' } },
};
