...
export const BLOCK_DEFS = {
  ...
  // MACHINE LEARNING
  'ml-train':         { label: 'Train model', icon: '🤖', color: '#673ab7', category: 'ml', params: { model: 'default' } },
...
export const SIDEBAR_TO_TYPE = {
  ...
  'when [loudness v] > ()': 'event-loudness', 'when loudness greater than': 'event-loudness',
  ...
  '[Hand] X position of': 'hand-position-x',
  ...
  // ML environment
  '[ml] train classifier (sim)': 'ml-train', '[ml] training window open': 'ml-train',
  ...
  '[qr] scan camera': 'object-analyse-camera', '[qr] last payload': 'sense-answer',
...
export function BlockContent({ block, onParamChange, context = {} }) {
  ...
  switch (block.type) {
    ...
    case 'ml-train': return <>{block.icon} Train{PI('model', 70)}</>;
