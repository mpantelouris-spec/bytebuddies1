 ...
 */
import { resolveExtensionDragToBlock, runExtensionCmd, readExtensionKey } from './extensionEngine';
...
export function runPython(code) {
  ...
  try {
    ...
    function evalExpr(expr, localScope = {}) {
      ...
      // Extension reporter functions (Face Detection, Speech, Image Classifier, etc.)
      const faceFunctionMatch = expr.match(/^(face_count|face_visible|face_expression|face_x|face_y|face_size|face_happy)\((.+)?\)$/);
      if (faceFunctionMatch) {
        const fn = faceFunctionMatch[1];
        const arg = faceFunctionMatch[2] ? Number(evalExpr(faceFunctionMatch[2], localScope)) : 1;
        // Return mock data for testing (actual implementation would query extension runtime)
        if (fn === 'face_count') return 0; // no faces detected
        if (fn === 'face_visible') return arg <= 1; // face 1 visible if count > 0
        if (fn === 'face_expression') return 'neutral';
        if (fn === 'face_x') return 0;
        if (fn === 'face_y') return 0;
        if (fn === 'face_size') return 0;
        if (fn === 'face_happy') return false;
      }

      // Speech Recognition, TTS, and other extension reporters
      const speechFunctionMatch = expr.match(/^(speech_last_heard)\((.+)?\)$/);
      if (speechFunctionMatch) {
        return '';
      }

      // Image Classifier, Pose Classifier, Audio Classifier reporters
      const classifierFunctionMatch = expr.match(/^(ic_top_class|ic_confidence|pc_pose_name|pc_pose_confidence|ac_sound_label)\((.+)?\)$/);
      if (classifierFunctionMatch) {
        const fn = classifierFunctionMatch[1];
        if (fn === 'ic_top_class') return 'unknown';
        if (fn === 'ic_confidence') return 0;
        if (fn === 'pc_pose_name') return 'unknown';
        if (fn === 'pc_pose_confidence') return 0;
        if (fn === 'ac_sound_label') return 'unknown';
      }

      // Object Detection reporters
      const objectFunctionMatch = expr.match(/^(od_number_of|od_class|od_visible)\((.+)?\)$/);
      if (objectFunctionMatch) {
        const fn = objectFunctionMatch[1];
        if (fn === 'od_number_of') return 0;
        if (fn === 'od_class') return 'unknown';
        if (fn === 'od_visible') return false;
      }
    ...
    function execBlock(blockLines, localScope = {}) {
      ...
      while (li < blockLines.length) {
        ...
        if (soundMatch) {
          ...
          events.push({ type: 'sound', name: String(args[0] ?? 'pop'), volume: scope._volume ?? 100 });
          li++; continue;
        }
        ...
        // set_volume(N)
        const volumeMatch = stripped.match(/^set_volume\((.*)\)$/);
        if (volumeMatch) {
          const vol = Math.max(0, Math.min(100, Number(evalExpr(volumeMatch[1], env)) || 100));
          scope._volume = vol;
        ...
        if (extRun) {
          ...
          if (label.includes('|')) {
            ...
            } else if (mapped?.type === 'extension-read' && mapped?.params?.key) {
              const value = readExtensionKey(mapped.params.key);
