...
import { createQrScannerApi, analyseSourceForQr } from './qrScannerRuntime.js';
...
const TRANSPARENCY_STATE = {
  ...
  qr: { currentOpacity: 1, targetOpacity: 1 },
};
...
let qrRuntimeApi = null;
function getQrRuntime() {
  if (!qrRuntimeApi) qrRuntimeApi = createQrScannerApi(S, TRANSPARENCY_STATE, logOut, setupGlobalDragListeners);
  return qrRuntimeApi;
}
...
const DRAG = {
  ...
  '[qr] turn video on stage with': { kind: 'run', cmd: 'qr|video|on|0' },
  '[qr] show bounding box': { kind: 'run', cmd: 'qr|bbox|show' },
  '[qr] hide bounding box': { kind: 'run', cmd: 'qr|bbox|hide' },
  '[qr] analyse image for qr code from camera': { kind: 'run', cmd: 'qr|analyse|camera' },
  '[qr] analyse image for qr from camera': { kind: 'run', cmd: 'qr|analyse|camera' },
  '[qr] is qr code detected?': { kind: 'read', key: 'qr.detected' },
  '[qr] get qr code data': { kind: 'read', key: 'qr.payload' },
  '[qr] get angle': { kind: 'read', key: 'qr.angle' },
  '[qr] x position of center': { kind: 'read', key: 'qr.pos|x|center' },
  '[qr] y position of center': { kind: 'read', key: 'qr.pos|y|center' },
  '[qr] scan camera': { kind: 'run', cmd: 'qr|analyse|camera' },
...
function setupGlobalDragListeners() {
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    if (S.qr.dragging) {
      S.qr.posX = e.clientX - S.qr.dragStartX;
      S.qr.posY = e.clientY - S.qr.dragStartY;
      getQrRuntime().layoutQrOverlay();
    }
  });
  ...
  document.addEventListener('touchmove', (e) => {
    ...
    if (S.qr.dragging) {
      S.qr.posX = touch.clientX - S.qr.dragStartX;
      S.qr.posY = touch.clientY - S.qr.dragStartY;
      getQrRuntime().layoutQrOverlay();
    }
  });
...
export function runExtensionCmd(cmdStr, output) {
  ...
  switch (`${a}|${b}`) {
    ...
    case 'qr|video': {
      const mode = c || 'off';
      if (mode === 'off') {
        getQrRuntime().stopQrLoop(output);
        return;
      }
      const trans = Math.max(0, Math.min(100, parseInt(parts[3], 10) || 0));
      S.qr.transparency = trans;
      const op = 1 - trans / 100;
      TRANSPARENCY_STATE.qr.targetOpacity = op;
      TRANSPARENCY_STATE.qr.currentOpacity = op;
      getQrRuntime().ensureQrLoop(output);
      if (S.qr.video) S.qr.video.style.opacity = String(op);
      logOut(output, `[QR] Video ${mode}, transparency ${trans}%`);
      return;
    }
    case 'qr|bbox': {
      S.qr.bboxShow = c === 'show';
      getQrRuntime().drawQrOverlay();
      logOut(output, `[QR] Bounding box ${S.qr.bboxShow ? 'shown' : 'hidden'}`);
      return;
    }
    case 'qr|analyse':
    case 'qr|scan':
      getQrRuntime().runQrAnalyseOnce(output);
      return;
    case 'log|add':
...
export function readExtensionKey(key) {
  ...
  switch (key) {
    ...
    case 'qr.detected':
      getQrRuntime().ensureQrLoop(null);
      return !!S.qr.detected;
    case 'qr.angle':
      return Number(S.qr.angle) || 0;
    case 'qr.payload':
      return S.qr.payload || '';
    case 'qr.x':
      return Number(S.qr.x) || 0;
    case 'qr.y':
      return Number(S.qr.y) || 0;
    case 'listen.last':
    ...
    default: {
      ...
      if (String(key).startsWith('qr.pos|')) {
        const [, axis, anchor] = String(key).split('|');
        return getQrRuntime().readQrPosition(axis, anchor);
      }
...
export function updateTransparencyOpacity() {
  ...
  if (S.qr.video && TRANSPARENCY_STATE.qr) {
    S.qr.video.style.opacity = String(TRANSPARENCY_STATE.qr.currentOpacity);
  }
...
/** Remove QR floating overlay (e.g. if it was wrongly attached to the sidebar). */
export function cleanupQrSidebarOverlay() {
  getQrRuntime().stopQrLoop(null);
}

/** Decode QR from the Game Builder stage camera (not a sidebar overlay). */
export async function analyseQrFromGameStage(cameraStateRef) {
  const output = [];
  const frame = cameraStateRef?.current?.webcam?.captureFrame?.();
  if (frame?.canvas) {
    await analyseSourceForQr(S, frame.canvas, output, logOut);
    return output[output.length - 1] || '';
  }
  const v = webcamManager.getVideoElement();
  if (v && v.readyState >= 2) {
    await analyseSourceForQr(S, v, output, logOut);
    return output[output.length - 1] || '';
  }
