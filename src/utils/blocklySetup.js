...
export const defineBytebuddiesBlocks = () => {
  ...
  try {
    ...
    Blockly.Blocks['event_whenflagclicked'] = {
      init: function() {
        const flagIcon = 'data:image/svg+xml,' + encodeURIComponent(
          '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><circle cx="12" cy="12" r="11" fill="#4CBF56"/><path fill="#fff" d="M8 7v10l2-1 2 1 5-2.5V9.5L12 11l-2-1-2 1z"/></svg>',
        );
        this.appendDummyInput()
            .appendField('when')
            .appendField(new Blockly.FieldImage(flagIcon, 24, 24, 'Green flag'))
            .appendField('clicked');
        this.setPreviousStatement(false, null);
        this.setNextStatement(true, null);
    ...
    // event_whenflagclicked — defined above (PictoBlox spec with green-flag icon)

    // Event blocks — use PictoBlox definitions above (#FFBF00). Do not redefine here (hue 324 renders purple).
    ...
    // QR Code
    const QR_COLOUR = '#b91c1c';
    Blockly.Blocks['qr_turn_video_on'] = {
      init: function() {
        this.appendDummyInput()
            .appendField('turn')
            .appendField(new Blockly.FieldDropdown([['on', 'on'], ['off', 'off']]), 'STATE')
        ...
        this.setPreviousStatement(true, null);
        this.setNextStatement(true, null);
        this.setColour(QR_COLOUR);
        this.setTooltip('Turn QR scanner video on or off with transparency');
      }
    };

    Blockly.Blocks['qr_bounding_box'] = {
      init: function() {
        this.appendDummyInput()
            .appendField(new Blockly.FieldDropdown([['show', 'show'], ['hide', 'hide']]), 'MODE')
            .appendField('bounding box');
        this.setPreviousStatement(true, null);
        this.setNextStatement(true, null);
        this.setColour(QR_COLOUR);
        this.setTooltip('Show or hide QR bounding box');
      }
    };

    Blockly.Blocks['qr_analyse_camera'] = {
      init: function() {
        this.appendDummyInput()
            .appendField('analyse image for QR code from camera');
        this.setPreviousStatement(true, null);
        this.setNextStatement(true, null);
        this.setColour(QR_COLOUR);
        this.setTooltip('Scan current camera frame for a QR code');
      }
    };

    Blockly.Blocks['qr_is_detected'] = {
      init: function() {
        this.appendDummyInput()
            .appendField('is QR code detected?');
        this.setOutput(true, 'Boolean');
        this.setColour(QR_COLOUR);
        this.setTooltip('True if a QR code was found');
      }
    };

    Blockly.Blocks['qr_get_data'] = {
      init: function() {
        this.appendDummyInput()
            .appendField('get QR code data');
        this.setOutput(true, null);
        this.setColour(QR_COLOUR);
        this.setTooltip('Text payload from last QR scan');
      }
    };

    Blockly.Blocks['qr_position'] = {
      init: function() {
        this.appendDummyInput()
            .appendField(new Blockly.FieldDropdown([
              ['x position', 'x'],
        ...
        this.setOutput(true, 'Number');
        this.setColour(QR_COLOUR);
        this.setTooltip('Stage position of QR code');
      }
    };

    Blockly.Blocks['qr_get_angle'] = {
      init: function() {
        this.appendDummyInput()
            .appendField('get angle');
        this.setOutput(true, 'Number');
        this.setColour(QR_COLOUR);
        this.setTooltip('Rotation angle of QR code in degrees');
      }
    };
    ...
    Blockly.Blocks['ml_train'] = {
      init: function() {
        this.appendDummyInput().appendField('[ML] Train classifier (sim)');
        this.setPreviousStatement(true, null);
        this.setNextStatement(true, null);
        this.setColour('#5eead4');
        this.setTooltip('Simulate training a classifier on collected examples');
      },
    };

    Blockly.Blocks['ml_training_open'] = {
      init: function() {
        this.appendDummyInput().appendField('[ML] Training window open');
        this.setPreviousStatement(true, null);
        this.setNextStatement(true, null);
        this.setColour('#5eead4');
        this.setTooltip('Open the ML training window (simulated)');
      },
    };

    // Chat & prompts (stack block for Game Builder scripts)
    Blockly.Blocks['bb_chat_ask'] = {
      init: function() {
        this.appendDummyInput()
            .appendField('ask coding helper')
            .appendField(new Blockly.FieldTextInput('How do I move the sprite?'), 'QUESTION');
        this.setPreviousStatement(true, null);
        this.setNextStatement(true, null);
        this.setColour('#fb7185');
        this.setTooltip('Ask the coding helper a question');
      },
    };

    Blockly.Blocks['chat_ask'] = Blockly.Blocks['bb_chat_ask'];
...
export const generateToolbox = () => {
  return `
    <xml xmlns="https://developers.google.com/blockly/xml">
      ...
      <category name="QR Code" colour="#b91c1c">
        <block type="qr_turn_video_on"></block>
        <block type="qr_bounding_box"></block>
        <block type="qr_analyse_camera"></block>
        <block type="qr_is_detected"></block>
        <block type="qr_get_data"></block>
        <block type="qr_position"></block>
        <block type="qr_get_angle"></block>
      </category>
