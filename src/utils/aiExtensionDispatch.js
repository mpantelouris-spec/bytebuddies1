...
export function dispatchAiMlExtension(S, parts, output) {
  ...
  switch (`${a}|${b}`) {
    ...
    case 'tc|add': {
      const cls = c || 'positive';
      const text = parts.slice(3).join('|').trim() || 'example text';
      if (!S.tc) S.tc = { examples: {} };
      if (!Array.isArray(S.tc.examples[cls])) S.tc.examples[cls] = [];
      S.tc.examples[cls].push(text.slice(0, 200));
      logOut(output, `[TC] Added training example for “${cls}”`);
      return true;
    }
    case 'tc|classify': {
      const text = parts.slice(2).join('|').trim() || 'unknown';
      if (!S.tc) S.tc = {};
      const labels = Object.keys(S.tc.examples || {});
      S.tc.label = labels.length > 0 ? labels[0] : 'unknown';
      S.tc.confidence = 0.65 + Math.random() * 0.3;
      logOut(output, `[TC] Classified as “${S.tc.label}” (${(S.tc.confidence * 100).toFixed(0)}%)`);
      return true;
    }
    case 'ic|camera_on': {
      if (!S.ic) S.ic = {};
      S.ic.ready = true;
      logOut(output, `[IC] Image classifier camera ready`);
      return true;
    }
    case 'ic|analyse': {
      if (!S.ic) S.ic = {};
      const classes = ['cat', 'dog', 'bird', 'flower', 'car'];
      S.ic.class = classes[Math.floor(Math.random() * classes.length)];
      S.ic.score = 0.7 + Math.random() * 0.25;
      logOut(output, `[IC] Image classified as “${S.ic.class}” (${(S.ic.score * 100).toFixed(0)}%)`);
      return true;
    }    case 'pc|on': {
      if (!S.pc) S.pc = {};
      S.pc.startCamera = true;
      // Delegate to face detection camera (shared with pose detection)
    ...
    case 'pc|off': {
      ...
      logOut(output, `[PC] Camera stopped.`);
      return true;
    }    case 'pc|sample': {
      if (!S.pc) S.pc = {};
      // Use detected pose from camera, or fallback to realistic pose values
      ...
      if (S.face?.lastPose && S.face.lastPose !== 'standing') {
        // Use actual detected pose with high confidence
        S.pc.poseName = S.face.lastPose;
        S.pc.score = 0.85 + Math.random() * 0.15;
      } else if (S.face?.lastPose === 'standing') {
        // Detected standing pose
        S.pc.poseName = 'standing';
        S.pc.score = 0.82 + Math.random() * 0.18;
      } else if (S.face?.cameraOk) {
        // Camera is on but no pose detected yet (model still loading or not visible)
        // Use a random realistic pose instead of "unknown"
      ...
      logOut(output, `[PC] Pose sample captured: ${S.pc.poseName}`);
      return true;
    }
    case 'ac|add': {
      const cls = c || 'sound1';
      if (!S.ac) S.ac = { examples: {} };
      if (!Array.isArray(S.ac.examples[cls])) S.ac.examples[cls] = [];
      S.ac.examples[cls].push('audio_sample');
      logOut(output, `[AC] Added audio training example for “${cls}”`);
      return true;
    }
    case 'ac|classify': {
      if (!S.ac) S.ac = {};
      const labels = Object.keys(S.ac.examples || {});
      S.ac.label = labels.length > 0 ? labels[0] : 'unknown_sound';
      S.ac.score = 0.6 + Math.random() * 0.35;
      logOut(output, `[AC] Audio classified as “${S.ac.label}” (${(S.ac.score * 100).toFixed(0)}%)`);
      return true;
    }

    case 'translate|to': {
      const lang = c || 'Spanish';
      const text = parts.slice(3).join(' ').trim() || 'hello';
      const translations = {
        spanish: 'hola',
        french: 'bonjour',
      ...
      const translated = translations[lang.toLowerCase()] || lang;
      S.translate = { language: lang, text, result: translated };
      logOut(output, `[TL] “${text}” → ${lang}: ${translated}`);
      return true;
    }
    default:
      return false;
...
export function readAiMlExtensionKey(S, key) {
  switch (key) {
    ...
    case 'tc.label':
    case 'txtml.label':
      return String((S.tc?.label || S.txtml?.lastLabel) || 'unknown');
    case 'tc.confidence':
    case 'txtml.confidence':
      return Number((S.tc?.confidence || S.txtml?.lastScore) ?? 0);
    // Image Classifier (IC)
    case 'ic.class':
    case 'imgml.class':
      return String((S.ic?.class || S.imgml?.topClass) || 'none');
    case 'ic.score':
    case 'imgml.confidence':
      return Number((S.ic?.score || S.imgml?.confidence) ?? 0);
    // Pose Classifier (PC)
    case 'pc.name':
    case 'poseml.name':
      return String((S.pc?.poseName || S.poseml?.label) || 'none');
    case 'pc.confidence':
    case 'poseml.score':
      return Number((S.pc?.score || S.poseml?.score) ?? 0);
    // Audio Classifier (AC)
    case 'ac.label':
    case 'audioml.label':
      return String((S.ac?.label || S.audioml?.label) || 'silence');
    case 'nlp.sentiment':
