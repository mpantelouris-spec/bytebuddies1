import { lookupSidebarType, shortTypeToBlocklyType } from './blocks';
...
export const BB_FULLY_IMPLEMENTED = new Set([
  ...
  // QR Code blocks
  'qr_turn_video_on', 'qr_bounding_box', 'qr_analyse_camera', 'qr_is_detected',
  'qr_get_data', 'qr_position', 'qr_get_angle',
  ...
  // ML environment
  'ml_train', 'ml_training_open',

  // Chat & NLP blocks
  'bb_chat_ask', 'chat_ask', 'nlp_analyse_sentiment', 'nlp_sentiment_value', 'nlp_is_positive',
...
export function isRegisteredBbBlockType(bb) {
  ...
  return /^bb_(sprite|motion|looks|sound|var|list|func|control|loop|logic|sense|action|game|physics|music|robot|event|body|hand|tts)_/.test(bb)
    || /^(face|object|qr|speech|translate|ocr|ic|pc|ac|tc|nlp|chat)_/.test(bb);
}
...
export function inferLibShapeFromLabel(label) {
  const short = lookupSidebarType(label);
...
export function libraryLabelFieldOverrides(blockLabel, blockType, shortType) {
  ...
  if (blockType === 'bb_chat_ask' || blockType === 'chat_ask') {
    fields.QUESTION = 'How do I move the sprite?';
  }
  if (blockType === 'bb_action_print') fields.MESSAGE = 'Hello!';
  return fields;
}
...
export function resolveBlocklyTypeForLibraryLabel(blockLabel, preferredType = null) {
  ...
  const key = String(blockLabel || '').trim().toLowerCase();
  const short = lookupSidebarType(blockLabel) || lookupSidebarType(key);
