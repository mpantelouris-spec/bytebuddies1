...
import MissionBlockEditor from './MissionBlockEditor';
...
const MISSION_CHECKS = {
  'space-1': {
    starterBlocks: [
      { id: 'countdown', label: 'countdown', icon: '⏱️', output: 'Countdown: 3... 2... 1...', uid: 1 },
      { id: 'systems', label: 'systems check', icon: '✓', output: 'Systems check complete', uid: 2 }
    ],
    check: (blocks, out) => {
      if (blocks.length < 5) return { ok: false, msg: `Need at least 5 blocks! You have ${blocks.length}.` };
...
export default function MissionMode({ onNavigate }) {
  ...
  useEffect(() => {
    if (activeMission) {
      ...
      if (check?.starterBlocks) {
        setMissionBlocks(check.starterBlocks);
      } else {
  ...
  const runAndCheck = (camp, mission) => {
    ...
    // Handle block-based missions
    if (check.starterBlocks) {
      const output = missionBlocks.map(b => b.output);
      setMissionOutput(output.map(text => ({ type: 'output', text })));
      const result = check.check(missionBlocks, output.join('\n'));
    ...
    // Handle Python missions (existing code)
      ...
      <div style={{ minHeight: '100vh', background: camp.bg, padding: '32px 24px', maxWidth: 800, margin: '0 auto' }}>
        ...
        {MISSION_CHECKS[mission.id] && (
          <div style={{ marginBottom: 24 }}>
            ...
            {MISSION_CHECKS[mission.id]?.starterBlocks ? (
              <MissionBlockEditor
                campaignId={camp.id}
