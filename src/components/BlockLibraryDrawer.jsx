              ...
              {activeCategory === cat.name && (
                <div className="block-library-drawer-blocks">
                  {cat.blocks.map((block, bi) => {
                    ...
                    const showScratchPreview = usePictoBlockStyle && isPaletteBlockEntry(blockName);

                    return showScratchPreview ? (
                      <button
                        key={`${cat.name}-${bi}-${block}`}
                        type="button"
                        className="block-library-drawer-block-row block-library-drawer-block-row--scratch"
                        onClick={() => pickBlock(block)}
                      >
                        <SidebarBlockPreview blockName={blockName} categoryName={cat.name} />
                      </button>
                    ) : (
                      <button
                        key={`${cat.name}-${bi}-${block}`}
                        ...
                        style={{
                          ...
                          borderLeftWidth: 0,
                          borderRadius: blockName.toLowerCase().startsWith('on ') ? '14px 14px 6px 6px' : 8,
                          color: '#fff',
