...
export default function BlocklySidebarFlyout({ categories = [] }) {
  ...
  useLayoutEffect(() => {
    ...
    const onChange = (evt) => {
      ...
      let name = block.getFieldValue('BLOCK_NAME') || block.getFieldValue('LABEL');
      if (!name && BLOCKLY_TYPE_TO_SIDEBAR_NAME[blockType]) {
        name = BLOCKLY_TYPE_TO_SIDEBAR_NAME[blockType];
      }
      if (!name && !Blockly.Blocks[blockType]) return;
      const colour = typeof block.getColour === 'function' ? block.getColour() : null;
      const resolvedType = resolveBlocklyTypeForLibraryLabel(String(name || ''), blockType);
      emitAddSidebarBlock(String(name || blockType), resolvedType, colour);
      block.dispose(false);
    };
