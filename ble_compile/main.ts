...
//   Direction: 1=one way, 2=other way, 0=stop  Speed: 0-100
//   fw/bk: dir=2/dir=1 (both motors same dir @80)
...
function runCmd(cmd: string): void {
    ...
    // fw: both dir=2 @80   bk: both dir=1 @80
    // lt: left=1,right=2 @60   rt: left=2,right=1 @60
    if (fn === "fw") {
    ...
    // Headlights — white LEDs, I2C: reg 0x04=left, 0x08=right, [reg, R, G, B]
    } else if (fn === "hl") {
        let buf = pins.createBuffer(4)
