...
export default function UnifiedBlocklyWorkspace({
  ...
  const lastSpriteNameRef = useRef(null);
  ...
  // Handle sprite switches: clear workspace and prevent emissions to avoid overwriting stored code
  useLayoutEffect(() => {
    const ws = workspaceRef.current;
    if (!ws) return;

    // Only take action if sprite name changed (not ID)
    if (selectedSpriteName !== lastSpriteNameRef.current) {
      lastSpriteNameRef.current = selectedSpriteName;

      console.log(`[UnifiedBlocklyWorkspace] Sprite switched to: "${selectedSpriteName}", has ${selectedSpriteBlocks?.length || 0} blocks`);

      // Temporarily remove listener to prevent sending empty workspace to parent
      if (onChangeRef.current) {
        ws.removeChangeListener(onChangeRef.current);
      }

      // Clear workspace
      ws.clear();

      // Recreate blocks if any exist for this sprite
      if (Array.isArray(selectedSpriteBlocks) && selectedSpriteBlocks.length > 0) {
        let recreateCount = 0;
        let errorCount = 0;

        selectedSpriteBlocks.forEach((gameBlock, idx) => {
          try {
            const blockType = gameBlock.type;

            // Defensive: Verify block type exists
            if (!blockType) {
              console.warn(`[UnifiedBlocklyWorkspace] Block at index ${idx} has no type field`);
              errorCount++;
              return;
            }

            if (!Blockly.Blocks[blockType]) {
              console.warn(`[UnifiedBlocklyWorkspace] Block type not found: "${blockType}"`);
              errorCount++;
              return;
            }
            ...
            if (!block) {
              console.warn(`[UnifiedBlocklyWorkspace] Failed to create Blockly block for type: "${blockType}"`);
              errorCount++;
              return;
            }

            // Set field values from parameters
            if (gameBlock.params && typeof gameBlock.params === 'object') {
              Object.entries(gameBlock.params).forEach(([key, value]) => {
                try {
                  block.setFieldValue(String(value), key);
                } catch (e) {
                  // Field might not exist in Blockly definition, that's OK
                  // console.debug(`Field ${key} not available for block type ${blockType}`);
                }
              });
            }

            // Position the block using stored coordinates
            const x = gameBlock.x || 30;
            const y = gameBlock.y || 30 + idx * 60;
            block.moveTo(x, y);
            block.initSvg();
            block.render();
            recreateCount++;
          } catch (e) {
            console.error(`[UnifiedBlocklyWorkspace] Error recreating block at index ${idx}:`, gameBlock.type, e);
            errorCount++;
          }
        });

        console.log(`[UnifiedBlocklyWorkspace] Recreated ${recreateCount} blocks for "${selectedSpriteName}", ${errorCount} errors`);

        // Verify blocks were actually created
        if (recreateCount === 0 && selectedSpriteBlocks.length > 0) {
          console.warn(`[UnifiedBlocklyWorkspace] WARNING: No blocks were recreated for sprite "${selectedSpriteName}"`);
        }
      } else {
        console.log(`[UnifiedBlocklyWorkspace] No blocks to restore for sprite "${selectedSpriteName}"`);
      }

      // Update hasBlocks state
      const topBlocks = ws.getTopBlocks(true);
      setHasBlocks(topBlocks.length > 0);
      console.log(`[UnifiedBlocklyWorkspace] Workspace now has ${topBlocks.length} top-level blocks`);

      // Re-add listener now that blocks are loaded
      if (onChangeRef.current) {
        ws.addChangeListener(onChangeRef.current);
      }
    }
  }, [selectedSpriteName, selectedSpriteBlocks]);
